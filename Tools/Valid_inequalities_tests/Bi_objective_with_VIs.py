...
# 读取数据
file_path = "E:/ToU重做3/dict_experiment_data/D_0.01/10_110.json"
...
def epsilon_constraint_solve(model,y,t,w,z,C_max,TEC,p):
    ...
    model.remove(TEC_vli1)
    model.remove(TEC_vli2)
    model.remove(TEC_vli3)
    # compute minimum Cmax
    constrs_v_i1 = model.addConstrs( quicksum(t[i,j,k] for j in range(n) for k in range(K)) <= C_max for i in range(m))
    constrs_v_i2 = model.addConstrs( C_max 
                     >= quicksum(p[i][j] * y[i, j] for j in range(n))  for i in range(m))
    constr_v_i3 = model.addConstr( C_max 
    ...
    z_lu = [0,0]
    z_rl = [0,0]
    model.remove(constrs_v_i1)
    model.remove(constrs_v_i2)
    model.remove(constr_v_i3)
    # solve the problem
    Pareto_set = set()
    solution_list = []
    # Delta2 = round( compute_Klow_Khigh_rate(K_low,epsilon2),1 )
    # print("Delta: ",Delta)
    # TEC_v_i = model.addConstr((Delta*quicksum(t[i,j,k] for k in K_low for j in range(n) for i in range(m)) 
    #                  - quicksum(t[i,j,k] for k in K_high for j in range(n) for i in range(m) )>= -0.5*epsilon2))
    ...
    # model.remove(model.getConstrByName("TEC_v_i"))
    # model.Params.MIPGap = 0.0047
    model.addConstrs( quicksum(t[i,j,k] for j in range(n) for k in range(K)) - C_max <=  epsilon for i in range(m))
    model.addConstrs( C_max 
                     >= quicksum(p[i][j] * y[i, j] for j in range(n)) - epsilon  for i in range(m) )
    model.addConstr( C_max 
                     >= 1/m * quicksum(p[i][j] * y[i, j] for j in range(n) for i in range(m)) )
    Updatable_constr = model.addConstr(TEC <= epsilon)
    model.setObjective(C_max, GRB.MINIMIZE)
    model.optimize()
    if model.status == GRB.Status.OPTIMAL or model.status == GRB.TIME_LIMIT:
        if model.SolCount >0:
...
result_file = "E:/ToU重做3/dict_experiment_data/Tools/Valid_inequalities_tests/Bio_results_D_0.01/PolicyB/10_110_result.json"
# dataset = load_dataset_from_file(result_file)
dataset = {}

dataset["pareto_set"] = pareto_set2
# dataset["time_cost"] = time_cost2
# dataset["avg_gap"] = avg_gap
dataset["solution_list"] = solution_list2
dataset["Time_vis_plus"] = time_cost2
dataset["Gap_vis_plus"] = avg_gap
print("P2_pareto_set: ",dataset["pareto_set"])
print("P2_time_cost: ",dataset["Time_vis_plus"])
print("P2_avg_gap: ",dataset["Gap_vis_plus"])
...
dict_result_str_keys = convert_keys_to_str(dataset)
